...
export default function Newpassword() {
  ...
  const validationSchema = Yup.object().shape({
    ...
    newPassword: Yup.string()
      .matches(new RegExp("^[A-Z][a-z0-9]{6,8}$"), "Invalid Password")
      .required("required"),
  });
  ...
  async function callNewPassword(values) {
    try {
      ...
      navigate("/login", { state: { email: values.email , newPassword:values.newPassword} });
  ...
  return (
    <>
      <form onSubmit={newPasswordForm.handleSubmit} className="w-1/2 mx-auto my-20">
        <div>
          {apiError ? (
            <div
        ...
        <div className="relative z-0 w-full mb-5 group">
          ...
          {/* // errors */}
          {newPasswordForm.errors.email && newPasswordForm.touched.email ? (
            <div
              className="p-2 mb-1 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400"
              role="alert"
            >
              <span classname="font-medium">{newPasswordForm.errors.email}</span>
            </div>
          ) : null}
        </div>
        {/* newPassword */}
        <div className="relative z-0 w-full mb-5 group">
          ...
          <label
            ...
            className="px-2 peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-8 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-main peer-focus:dark:text-main peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-5 peer-focus:-translate-y-8"
          >
            Enter your newPassword address
          </label>

          {/* // errors */}
          {newPasswordForm.errors.newPassword && newPasswordForm.touched.newPassword ? (
            <div
              className="p-2 mb-1 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400"
              role="alert"
            >
              <span classname="font-medium">{newPasswordForm.errors.newPassword}</span>
            </div>
          ) : null}
        </div>
